    ...
    public function store(Request $request)
    {
        ...
        $rules = [
            'title' => [
                ...
                function ($attribute, $value, $fail) {
                    if (str_word_count($value) >= $this->max_title) {
                        $fail(ucfirst($attribute) . ' is more than ' . $this->max_title . ' words');
                    }
                },
            ],
            'content' => [
        ...
        if (!$review) {
            ...
            if (!$review) {
                ...
                return \redirect()->to('company/' . $post->slug)->with('danger', 'Cannot edit approved review.');
            }

            Review::find($review->id)->update($fields);
            $msg = 'Review has been updated.';
        }
        return \redirect()->to('company/' . $post->slug)->with('info', $msg);
    }
}
